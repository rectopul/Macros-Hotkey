...
;     PixelGetColor, colorRing, 1776, 282

;     MouseMove, 1776, 282

;     if(colorRing="0x2D893C"){
;         MsgBox ColorRing na Posição atual é %colorRing%.
;     }
...
/*
active_id:= WinExist("ahk_exe client.exe") 
pixels:= "1776, 282"
 Display_CreateWindowCapture(device, context, pixels, active_id)
 pixcol:= Display_GetPixel(context, 1776, 282)
 rgbvalue:=bgr2rgb(SubStr("000000" . SubStr(pixcol + 0, 3),-5))
Gui,color,%rgbvalue%
Gui, Show,w150 h150 x150 y150 , rgbcolor
 msgbox, %rgbvalue%
 */

gui destroy
MouseGetPos, x,y,win_handle
ToolTip, %x%  %y% handle= %win_handle%
return 


f4::

Display_CreateWindowCapture(device, context, pixels, win_handle)
pixcolor:= Display_GetPixel(context, x,y)

SetFormat, integer,hex
rgbvalue:=bgr2rgb(SubStr("000000" . SubStr(pixcolor + 0, 3),-5))
